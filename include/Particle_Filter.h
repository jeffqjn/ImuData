...
public:
    ...
    void transform_use_particle(pcl::PointCloud<pcl::PointXYZ> pointcloud, Eigen::Vector3d &angle, Eigen::Vector3d &translation,pcl::PointCloud<pcl::PointXYZ> &estimation);
    ...
    IntervalVector create_6D_box(IMU imu, double start_time, double end_time);
    int calculate_weight(LiDAR_PointCloud &pointcloud, vector<int> &indices, vector<bool> &if_matched, IntervalVector & point_after_transform,int k);
    ...
    void build_LiDAR_Interval(Parameters parameters,double timestamp,pcl::PointCloud<pcl::PointXYZ> &pc);
